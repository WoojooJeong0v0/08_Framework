...
/** 댓글 목록 조회 함수 (비동기 ajax)
 * 
 */
const selectCommentList = () => {

  // boardNo 변수는 어디에? 
  // boardDetail.js 맨 위에 전역변수 선언
  fetch("/board/commentList?boardNo=" + boardNo)
  // GET방식만 쿼리스트링으로 전달을 한다!
  .then(response=>{
...
const insertComment = (parentCommentNo) => {
  ...
  data.boardNo = boardNo; // 댓글 작성된 게시글 번호 (전역 변수에 있음!)
  data.commentContent = commentContent.value; // 작성된 댓글 내용
  // 회원번호는 session에서 받아오기
  ...
  .then(response => {
    if(response.ok) return response.text(); // text로 파싱
  ...
  .then(commentNo => {
    ...
    commentContent.value = ""; // textarea에 작성한 댓글 내용을 삭제 (화면에 남아있지 않게)
    selectCommentList(); // 댓글 목록 비동기 조회 후 출력 // 화면 전체 다시 만들기
...
const showChildComment = (btn) => {
  ...
  // closest("태그") : 부모 중 가장 가까운 태그 찾는 함수
  const li = btn.closest("li");
  
  // 답글 작성되는 댓글(부모 댓글) 번호 얻어오기
...
const deleteComment = (btn) => {
  ...
  // 삭제할 댓글 번호 얻어오기
  const li = btn.closest("li"); // 댓글 또는 답글 (하나)
  ...
  .then(result => {
    if(result > 0){
      alert("삭제 되었습니다");
       selectCommentList(); // 댓글 비동기 목록 조회해서 출력
    } else {
      alert("삭제 실패");
    }
  })
...
const showUpdateComment = (btn) => {
  ...
  /* 댓글 수정 화면이 1개만 열리도록 하기 */
  // == 이미 열려있는 수정 화면이 있으면 닫기

  const temp = document.querySelector(".update-textarea");
  if(temp !== null){ // 이미 열린 수정 화면이 있을 경우

    if(confirm("수정 중인 댓글이 있습니다. 현재 댓글을 수정하시겠습니까?") === true){
      const commentRow = temp.parentElement; // 열린 댓글행
      commentRow.after(beforeCommentRow); // 백업본 다음 요소로 추가
      commentRow.remove(); // 열렸던 행 삭제 (취소 버튼과 같은 동작)
  ...
  // 2. 취소 버튼 동작에 대비하여 현재 댓글(comment row)의 요소 복제(html 코드) 백업하기

  beforeCommentRow = commentRow.cloneNode(true);
  /* 요소.cloneNode(true)
  - 요소를 복제하여 반환
  - 매개변수 true : 복제하려는 요소의 하위 요소들을 복제
  */
  ...
  cancelBtn.addEventListener("click", ()=>{
    ...
    // 수정 취소 원래 상태로 -> 현재 댓글 행 다음 위치에 백업한 원본 댓글 추가
    commentRow.after(beforeCommentRow);
    commentRow.remove(); // 수정화면으로 전환된 행 삭제

    /* 원상 복구 된 댓글 버튼에 이벤트 추가하기 */
    const childCommentBtn = beforeCommentRow.querySelector(".child-comment-btn");
    // beforeCommentRow (복구)된 애들 중에서 선택해! 특정 지을 수 있음
    ...
    // 부품을 함수로 따로 만들어서 필요한 곳에 가져다 쓰는 [모듈화] 설계
...
addComment.addEventListener("click", ()=>{
  ...
  // 1) 로그인 여부 검사 (boardDetail.html 의 loginCheck 전역변수)
  if(loginCheck === false){
    alert("로그인 후 이용해 주세요");
    return;
  }
  ...
  // 3) 1, 2번 통과 시 댓글 등록 함수 호출
  insertComment();
})

/* 화면에 존재하는 답글 버튼을 모두 찾아서 이벤트 리스너 추가 */
const addEventChildComment = () => {
  const btns = document.querySelectorAll(".child-comment-btn");

  btns.forEach(btn => {
    btn.addEventListener("click", ()=>{
      showChildComment(btn); // 답글 작성 화면 출력 함수 호출 // 클릭된 답글 버튼을 매개변수로
    });
  })
}
...
/**
 * 화면에 존재하는 모든 댓글 삭제 버튼에 이벤트 리스너 추가 하는 함수
 */
...
/**
 * 화면에 존재하는 댓글 수정 버튼에 이벤트 리스너 추가하기
 */
...
/* 답글(대댓글) 버튼 클릭 동작 */
/* 모든 화면 코드 해석 완료 후 (화면 로딩 끝났을 때) */
document.addEventListener("DOMContentLoaded", ()=>{
  addEventChildComment(); // 답글 버튼에 이벤트 추가해!
  addEventDeleteComment(); // 삭제 버튼에 이벤트 추가
  addEventUpdateComment(); // 수정 버튼에 이벤트 추가
});
...
// --------------------------------------------
// Restful 잘 지켜진! 아래의 REST API를 잘 지킨!
// 이 방법을 얼마나 잘 지켰느냐~ 하는 뜻으로 확인 필요
