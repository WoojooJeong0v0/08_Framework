...
// Required : 필수
// 필드 중 **final 키워드** 작성된 필드 값을 초기화하기 위한 
// **생성자**를 자동완성하는 Lombok 제공 어노테이션 

// + final 필드 초기화를 위한 생성자 매개변수에
// 자동으로 같은 자료형 Bean을 의존성 주입함
...
public class EmailServiceImpl implements EmailService {
	...
	// 필드 선언, 상수형 필드
	private final JavaMailSender mailSender; // EmailConfig 내용이 적용된 이메일 발송 객체(Bean)
	
	private final RedisUtil redisUtil; // Redis(inMemory DB)에 값을 CRUD 할 수 있는 기능 제공하는 객체(Bean)
	
	private final SpringTemplateEngine templateEngine; // Java에서 타임리프 사용할 수 있게 하는 객체 (Bean)
	...
	public int sendEmail(String htmlName, String email) {
		...
		try {
			...
			String emailTitle = null; // 발송되는 이메일 제목 역할 변수
			String authKey	  = createAuthKey(); // 생성된 인증 번호(아래 메서드 호출)
			
			// 이메일 발송 시 사용할 html 파일 이름에 따라
			// 이메일 제목, 내용을 다르게 설정
			switch(htmlName) {
			case "signUp" : 
				emailTitle = "[boardProject] 회원 가입 인증번호입니다.";
				break;
			case "findPw" :
			...
			// MimeMessageHelper Spring 제공
			// 메일발송 도우미
			// 매개변수 1 : MimeMessage
			// 매개변수 2 : 이메일 파일 첨부 여부
			// 매개변수 3 : 발송되는 이메일 문자 인코딩 지정 "UTF-8"
			...
			// 매개변수 1 : 이메일 내용
			// 매개변수 2 : html 코드 해석 여부 지정 (true > 해석해라)
			
			// 지정된 HTML 파일에 authKey 가 첨부된 후
			// HTML 코드 전체가 하나의 String으로 변환되어 반환받고
			// 변환된 String을 메일 내용으로 세팅한다는 뜻!
			
			// CID(Content-ID) 이용해서 메일에 이미지 첨부
		...
		} catch (Exception e) { // 예외 발생 == 실패 == 0 반환
			e.printStackTrace();
		}
		
		return 1; // 예외발생 X == 성공 == 1 반환
	}
  ...
  public String createAuthKey() {
	...
	public boolean checkAuthKey(Map<String, String> map) {
		...
		if(redisUtil.hasKey(email) == false) { // 같은 데이터가 없을 경우
			return false;
		}
...
/* 추천 */
// 의존성 주입 방법 3 : 생성자 이용 (필드 많을 때)
/*
 * private 자료형 필드명1; private 자료형 필드명2;
 * 
 * @Autowired public 클래스명(자료형 변수명1, 자료형 변수명2) { 
 * this.필드명1 = 변수명1; 
 * this.필드명2 = 변수명2; }
 */