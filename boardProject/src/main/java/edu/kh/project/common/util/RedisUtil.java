...
public class RedisUtil {
  ...
  // StringRedisTemplate 
  // - Redis 에서 문자열 데이터를 쉽게 처리할 수 있는 클래스 
  // - RedisConfig에서 Bean 으로 등록된 RedisTemplate을 의존성 주입
  @Autowired
  private StringRedisTemplate redisTemplate;

  // Redis 에서 키에 해당하는 값을 가져오는 메서드
  public String getValue(String key) {
    return redisTemplate.opsForValue().get(key);
  }

  // Redis 에 키와 값을 저장하는 메서드, 값의 만료 시간도 설정
  ...
  // Redis 에서 키에 해당하는 값을 삭제하는 메서드
  public void deleteValue(String key) {
    redisTemplate.delete(key);
  }

  // Redis 에 키가 존재하는지 확인하는 메서드
