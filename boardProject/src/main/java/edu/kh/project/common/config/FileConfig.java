 ...
 * org.springframework.util.unit.DataSize; import
 ...
 * 
 * @Configuration // 서버 실행 시 해당 클래스 내부 메서드를 모두 실행한다
 * 
 * @PropertySource("classpath:/config.properties") // config.properties에 작성된 내용을
 * 얻어와 사용하겠다 public class FileConfig implements WebMvcConfigurer {
 * 
 * // config.properties에서 ${key}가 일치하는 값 얻어와 변수에 대입
 * 
 * @Value("${spring.servlet.multipart.file-size-threshold}") private long
 * fileSizeThreshold; // 임계값
 * 
 * @Value("${spring.servlet.multipart.max-request-size}") private long
 * maxRequestSize; // 요청당 파일 최대 크기
 * 
 * @Value("${spring.servlet.multipart.max-file-size}") private long maxFileSize;
 * // 개별 파일당 최대 크기
 * 
 * @Value("${spring.servlet.multipart.location}") private String location; //
 ...
 * // test 관련 값
 * 
 * @Value("${my.test.resource-handler}") private String testResourceHandler; //
 * 테스트 이미지 요청 주소
 * 
 * @Value("${my.test.resource-location}") private String testResourceLocation;
 * // 테스트 이미지 요청 시 연결될 서버 폴더 경로
 * 
 * 
 * MultipartResolver 설정
 * 
 * @Bean public MultipartConfigElement configElement() {
 * 
 ...
 * 
 * 
 * // MultipartResolver 객체를 bean으로 추가 // -> 추가 후 위에서 만든 MultipartConfig를 자동으로 이용
 * 
 ...
 * 
 * 
 * // 인터넷(웹)으로 특정 형태 요청(js나 css, 이미지 등의 정적자원 static) 이 있을 경우 // 서버 컴퓨터의 특정 폴더와
 * 연결하는 설정 // 을 작성하는 메서드
 * 
 * @Override public void addResourceHandlers(ResourceHandlerRegistry registry) {
 * registry.addResourceHandler(testResourceHandler).addResourceLocations(
 * testResourceLocation);
 * 
 * // /images/test/ 로 시작하는 주소로 요청 있을 경우 // 서버 컴퓨터에 C:/uploadFiles/test/ 폴더로 연결 }
 * 
 * 
 * 
 * }
