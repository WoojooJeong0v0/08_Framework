...
public class BoardServiceImpl implements BoardService {
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		// 예 ) cp == 1,  전체 목록 중 1 ~  10행 결과만 반환
		// 			cp == 2,  전체 목록 중 11 ~ 20행 결과만 반환
		//      cp == 10, 전체 목록 중 91 ~ 100행 결과만 반환
		/* [RowBounds 객체] SQL에서는 복잡해서 mybatis 에서 진행 할 수 있음
		 * - Mybatis 제공 객체
		 * - 지정된 크기(offset) 만큼 행을 건너 뛰고 제한된 크기(limit) 만큼 행을 조회
		 * 
		 * - 사용법 : Mapper 메서드호출 시 
		 * 						2 번째 이후 매개변수로 전달
...
//		 = Map.of("boardList", boardList, "pagination", pagination);
	...
	public Board selectDetail(Map<String, Integer> map) {
		 ...
		 * 단점 : service <-> mapper <-> DB 많이 수행...
		 * (필요한 SELECT 만큼 전체 반복)
		 * 
		 * 2. MyBatis 에서 제공하는 <resultMap>, <collection> 이용하기
		 * service -> mapper (service 로 돌아오는 거 없이 SELECT 연속 수행) -> DB
		 * 
		 */
	...
	public Map<String, Object> boardLike(int boardNo, int memberNo) {
		...
		// 3. INSERT, DELETE 성공 시 해당 게시글 좋아요 개수 조회
		int count = 0;
		if(result2 > 0) {
			count = mapper.getLikeCount(boardNo);
		} else return null; // INSERT, DELETE 실패
